...
async function setupTestContext(browser: any): Promise<{ context: BrowserContext; page: Page }> {
  ...
  await page.goto('https://dev.localecash.com/init');
...
test.skip('Create offer', async ({ browser }) => {
...
test.skip('Create and cancel order', async ({ browser }) => {
  const { context, page } = await setupTestContext(browser);
  try {
    // Test implementation...
    // Go to offers page and wait for it to load
    // Get the default context and first page
    const context = await browser.contexts()[0];
    const page = context.pages()[0] || await context.newPage(); // Use an existing page if available, or create a new one if none exist
    
    await page.goto('https://dev.localecash.com/offer-detail?id=cm5d70ia5001xh17omwnensio');
    ...
    const amountPlaceholder = await page.locator('input[name="amount"]').getAttribute('placeholder');
    const [minAmount, maxAmount] = amountPlaceholder.match(/\d+/g).map(Number);
    const randomAmount = (Math.random() * (maxAmount - minAmount) + minAmount).toFixed(2);
    await page.locator('input[name="amount"]').click();
    await page.locator('input[name="amount"]').fill('');
    await page.locator('input[name="amount"]').type(randomAmount, { delay: 100 });
    
    await page.locator('textarea#message').click();
    await page.locator('textarea#message').fill('I want to buy XEC via bank transfer.');
    await page.getByRole('button', { name: 'Create' }).click();    // Find either 'No, thanks' or 'Deposit' button
